    ...
    <>
      ...
      <div style={css("position:fixed;left:50%;top:50%;z-index:901;transform:translate(-50%,-50%);width:560px;max-width:94vw;background:var(--onair-surface);border-radius:16px;box-shadow:var(--sh-modal);animation:modalIn .3s var(--ease-out);overflow:hidden")}>
        ...
        <div style={css("display:flex;align-items:center;gap:9px;padding:16px 20px 12px")}>
          ...
          <span style={css("font:700 15px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-1000)")}>규정 PDF 자동 적재</span>
          <span style={css("font:400 11.5px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-600)")}>· 업로드 즉시 검색에 반영</span>
          ...
          {phase.kind === "idle" && (
            <>
              ...
              >
                ...
                <span style={css("font:600 13.5px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-1000)")}>
                  PDF를 끌어다 놓거나 클릭해서 선택
                </span>
                <span style={css("font:400 11.5px/1.6 'Avenir Next','Pretendard',sans-serif;color:var(--gray-700);text-align:center")}>
          ...
          {phase.kind === "busy" && (
            <div style={css("display:flex;flex-direction:column;gap:12px;padding:6px 2px")}>
              <div style={css("display:flex;align-items:center;gap:9px")}>
                ...
                <span style={css("font:700 13px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-1000)")}>{phase.filename}</span>
                <span style={css("font:400 11.5px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-600)")}>처리 중 — 문서 크기에 따라 수십 초</span>
              </div>
          ...
          {phase.kind === "error" && (
            <div style={css("display:flex;flex-direction:column;gap:12px;padding:6px 2px")}>
              <div style={css("display:flex;gap:9px;align-items:flex-start;background:var(--background-200);border-radius:10px;padding:12px 14px")}>
                ...
                <span style={css("font:400 12.5px/1.6 'Avenir Next','Pretendard',sans-serif;color:var(--red-900)")}>{phase.message}</span>
              </div>
              <div style={css("display:flex;gap:8px;justify-content:flex-end")}>
                <span onClick={() => setPhase({ kind: "idle" })} style={css("display:inline-flex;align-items:center;gap:5px;border-radius:9999px;padding:8px 15px;font:600 12.5px 'Avenir Next','Pretendard',sans-serif;background:var(--blue-700);color:#fff;cursor:pointer")}>
  ...
  return (
    <div style={css("display:flex;flex-direction:column;gap:12px;padding:2px")}>
      <div style={css("display:flex;align-items:center;gap:9px")}>
        ...
        <div>
          <div style={css("font:700 13.5px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-1000)")}>
            {result.title}
            {result.is_scanned && (
              <span style={css("margin-left:7px;font:700 10px 'Avenir Next','Pretendard',sans-serif;color:var(--amber-900)")}>스캔본 · OCR</span>
            )}
          </div>
      ...
      <div style={css("background:var(--background-200);border-radius:10px;padding:11px 14px")}>
        <div style={css("display:flex;align-items:center;gap:7px;flex-wrap:wrap")}>
          <span style={css("font:600 11px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-700)")}>자동 분류</span>
        ...
        {result.suggestion.why.length > 0 && (
          <div style={css("margin-top:7px;display:flex;align-items:center;gap:5px;flex-wrap:wrap")}>
            <span style={css("font:600 10px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-600)")}>근거</span>
            {result.suggestion.why.slice(0, 6).map((w) => (
              <span key={w} style={css("font:500 10px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-800);background:var(--gray-100);border-radius:9999px;padding:2px 8px")}>{w}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <div style={css("display:flex;gap:9px;align-items:center;background:var(--background-200);border-radius:10px;padding:10px 13px")}>
        ...
        <span style={css("font:400 11.5px/1.55 'Avenir Next','Pretendard',sans-serif;color:var(--green-900)")}>
          지금부터 검색에 반영됩니다 — 상담사 규정검색과 카드 라우팅의 부서 필터가 이 문서를 바로 참조합니다.
        </span>
      </div>

      <div style={css("display:flex;gap:8px;justify-content:flex-end")}>
        <span onClick={onMore} style={css("display:inline-flex;align-items:center;gap:5px;border:1px solid var(--color-border);border-radius:9999px;padding:8px 15px;font:600 12.5px 'Avenir Next','Pretendard',sans-serif;color:var(--gray-900);cursor:pointer;background:var(--onair-surface)")}>
          <span className="mi" style={css("font-size:15px")}>add</span>다른 PDF 업로드
        </span>
        <span onClick={onClose} style={css("display:inline-flex;align-items:center;border-radius:9999px;padding:8px 17px;font:600 12.5px 'Avenir Next','Pretendard',sans-serif;background:var(--gray-1000);color:#fff;cursor:pointer")}>
          완료
        </span>
      </div>
    </div>
