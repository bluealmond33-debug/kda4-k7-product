  ...
  return (
    <div className="card" style={css("flex:1;display:flex;align-items:center;gap:16px;padding:12px 18px")}>
      ...
      <span style={css("display:flex;flex-direction:column;gap:5px;min-width:0")}>
        ...
        <span style={css("font:400 10.5px 'Geist Mono',monospace;color:var(--gray-600);white-space:nowrap")}>
          dense 0.65 + keyword 0.35 · bge-m3 1024d · HNSW
        </span>
      </span>
      <div style={css("flex:1")} />
      {/* 규정 PDF 업로드 — 청킹→추천→임베딩→적재→검색 반영까지 한 번에 */}
      <span
        onClick={onOpenUpload}
        style={css("flex:none;display:inline-flex;align-items:center;gap:6px;border-radius:9999px;padding:8px 15px;font:600 12.5px 'Geist Sans','Pretendard',sans-serif;background:var(--gray-1000);color:#fff;cursor:pointer")}
      >
        <span className="mi" style={css("font-size:16px")}>upload_file</span>규정 PDF 업로드
      </span>
    </div>
  );
}
