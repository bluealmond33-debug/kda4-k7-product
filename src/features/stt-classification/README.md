...
이 모듈의 결과를 PostgreSQL에 직접 저장하지 않습니다. FastAPI가 모델 결과를 JSON Schema로 검증하고 `database/adapters/model_result_adapter.py`로 상담카드·라우팅 계약으로 변환한 뒤 저장합니다.
...
모델 실행 백엔드가 분류 결과를 반환하는 예시입니다. 반환 결과는 FastAPI의 `POST /api/v1/consultation-sessions/{external_session_key}/model-results`로 전달합니다.
...
OpenAI API 키는 브라우저 코드에 넣지 않고 백엔드 환경변수로 관리합니다. 백엔드에서는 `prompt.ts`의 `classificationInstructions`와 `classificationResponseSchema`를 구조화 출력 설정에 사용합니다. 화면은 이 원시 결과를 직접 읽지 않고 통합 상담카드 조회 API만 사용합니다.
