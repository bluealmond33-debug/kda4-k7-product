...
/** 통화 연결 직후 — 인사→AI 안내→말씀 요청→전달 안내→# 안내를 한 테이크로 잇는다.
 *
 *  2026-07-29 1차 개정: 기존 문구가 "AI 상담 도우미가 안내해 드립니다"처럼 정체를 밝히기만
 *  하고 위 파일 상단의 문구 원칙(No ARS를 가장 먼저·전달 약속·콜백 안심)을 실제로 말하지
 *  않아 일반적인 IVR 인사말과 구분이 안 된다는 피드백으로 고쳤다.
 *
 *  2026-07-29 2차 개정("더 쎄게"): 이 한 줄만 듣고도 KARI-NA가 뭘 하는 서비스인지 알 수
 *  있어야 한다는 요청으로 두 가지를 더 얹었다 — ①고객 화면(Phone.tsx)의 발신자 이름이 이미
 *  "키움은행 KARI-NA"이므로 음성도 같은 이름("카리나")으로 자기소개해 화면·음성 정체성을
 *  맞췄다. ②발표 문서의 핵심 태그라인("개인정보는 건물을 나가지 않습니다")을 그대로 옮겨
 *  왔다 — 데모 환경이 실제로 폐쇄 LAN·로컬 모델만 쓰므로 과장이 아니다. 문장을 길게 잇지
 *  않고 짧게 끊어 "쎄게" 들리도록 했다. 1차 개정과 길이가 비슷해 sec/재녹음 필요성은 그대로다. */
export const ARS_GREETING: ArsLine[] = [
  {
    id: "greet-1",
    text: "안녕하세요, 키움은행 AI 상담 도우미 카리나입니다. 번호는 누르지 않으셔도 됩니다. 지금 하시는 말씀은 이 건물 밖으로 나가지 않고, 상담사에게 바로 전달됩니다. 통화가 끊기셔도 말씀은 사라지지 않습니다. 편하게 말씀해 주시고, 다 마치시면 우물 정자를 눌러 주세요.",
    sec: 19,
