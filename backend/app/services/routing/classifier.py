...
# local_llm.classify_task_with_llm()의 후보 목록 전용 — G004(기타)로 떨어진 걸 G0xx
# 안에서만 재분류하려는 함수라 SIMPLE(S) 코드는 애초에 후보일 필요가 없다(2026-07-28:
# 전체 TASK_NAMES를 후보로 줬다가, 우리카드 ARS 전용 카탈로그(S101~127, 도메인이 아예
# 다른 카드 업무)가 은행 자동이체 문의에 잘못 붙는 사고가 있었다).
#
# EMERGENCY(E001·E002)는 여기 포함한다 — S 코드와 달리 도메인이 섞이는 문제가 없고,
# 오히려 규칙 기반 긴급 판정(classify_transcript 상단)이 "보내라고 해요"처럼 자연어
# 표현을 놓쳐 G004로 떨어뜨린 사건성 발화를 이 LLM 보완이 다시 건져올릴 수 있는
# 유일한 경로다. S만 빼고 E는 남긴다.
GENERAL_TASK_NAMES = {
    task["code"]: task["name"] for task in GENERAL_TASKS + EMERGENCY_TASKS
}
...
SENSITIVE_DEMANDS = [
    ...
    # "송금"/"이체" 격식체 대신 "보내라고 해요"처럼 자연어로 신고하는 경우를 놓쳐
    # G004로 떨어졌던 사례(2026-07-28) — actors(사칭 주체)와 함께 있어야만 발동하는
    # 조건이라 "보내"류 단독으로는 오탐 위험이 낮다.
    "보내라고", "보내달라고", "돈을 보내", "입금하라고", "입금해달라고",
]
THREAT_OR_URGENCY = [
...
TASK_KEYWORDS = {
    ...
    "G002": [
        ...
        "대출 알아보", "대출 받을 수 있", "대출 문의", "대출 가능한지",
    ],
    "G003": [
        "계좌 제한", "계좌 이용 제한", "계좌 정지", "계좌가 정지", "거래 정지", "거래가 정지",
        "거래 제한", "계좌 잠", "압류",
        "계좌가 막혔", "계좌 풀어주세요", "한도 초과로 막힌", "비대면 한도 계좌",
    ],
    ...
    # "자동이체"라는 단어 없이 "다른 계좌에서 빠져나가도록 변경하고 싶거든요"처럼 말하는
    # 경우가 실제로 있다(2026-07-28 Windows 시연 보고: 규칙 매칭이 전부 실패해 G004로
    # 떨어졌고, 그 뒤 LLM 보완 분류가 카드 전용 S126(자동납부)로 잘못 골랐다). 계좌
    # 출금/이체를 "바꾸는" 자연어 표현을 보강한다. "빠져나가도록/빠져나가게"처럼 미래·
    # 희망형 어미만 잡아서, 과거형("빠져나갔어요" = 이미 벌어진 미승인 출금 신고, E002
    # 영역)과는 겹치지 않게 했다 — EMERGENCY 판정이 먼저 돌긴 하지만 이중 안전장치.
    "G006": [
        "자동이체", "정기이체",
        "출금계좌 변경", "출금 계좌 변경", "출금계좌를 변경", "출금 계좌를 변경",
        "출금계좌 바꾸", "출금 계좌 바꾸", "출금계좌를 바꾸", "출금 계좌를 바꾸",
        "이체 계좌 변경", "이체 계좌를 변경", "이체 계좌 바꾸", "이체 계좌를 바꾸",
        "빠져나가도록 변경", "빠져나가게 변경", "빠지도록 변경",
        "매달 빠져나가는", "다달이 빠져나가는", "자동으로 빠져나가",
    ],
    "G007": [
    ...
    "G008": ["연체", "이자가 얼마", "이자는 얼마", "이자 얼마", "연체료", "이자 얼마나"],
    "G009": [
