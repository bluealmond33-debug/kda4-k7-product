...
_DOCS: list[dict] = [
    ...
    # 실물 규정 1,153청크 중 "자동이체" 언급은 주택청약(납입용)·대출(상환용)뿐이라, 아파트
    # 관리비·공과금·통신비 같은 일반 자동이체 출금계좌 변경 문의에 맞는 근거가 하나도 없었다
    # (현장 확인: 이 문의가 '아파트'→주택청약으로 새거나 crd-004 카드자동납부로 잘못 감).
    # 실물 PDF에 해당 약관이 도착하기 전까지의 데모용 근거 자리.
    {
        "doc_id": "dep-002", "category": "DEP", "subcategory": "자동이체계좌변경",
        "title": "자동이체(공과금·관리비 등) 출금계좌 등록·변경·해지",
        "text": (
            "아파트 관리비, 공과금, 통신비 등 정기적으로 빠져나가는 일반 자동이체의 출금계좌를 "
            "등록·변경·해지하는 절차다. 카드대금 자동납부(CRD)나 주택청약 정기납입과는 별개의 "
            "일반 계좌 자동이체 서비스로, 기존 출금계좌를 확인한 뒤 새 계좌로 변경 신청하면 "
            "다음 결제 주기부터 반영됨을 안내한다. 변경 신청 시점이 출금일에 임박하면 이번 회차는 "
            "기존 계좌에서 먼저 빠져나갈 수 있어 신청 시점과 적용 시점을 함께 고지한다."
        ),
    },
    # ── FX(외환·수출입) ──
    {
...
_DEPT_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    "DEP": ("예금", "적금", "청약", "통장", "입출금", "발행어음", "만기", "수신", "자동이체", "공과금", "관리비"),
}
