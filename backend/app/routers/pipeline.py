...
# 같이 날아간다). 매핑에 없는 부서(수신·예적금 등)는 필터 없이 전체 검색한다.
# 부서명은 app/routing/taxonomy.py의 공식 DEPARTMENTS와 동일해야 한다(박정운 피드백:
# LLM이 지어낸 별도 부서명 대신 전형진 S/G/E taxonomy로 통일).
_DEPT_RAG_CATEGORIES: dict[str, list[str]] = {
    "사고·신고": ["SG", "EFN", "CRD"],      # 보이스피싱·명의도용·계좌보안 등 긴급 사고 전반
    "카드·결제": ["CRD", "SG"],
    "전자금융·디지털": ["EFN", "SG"],
    "여신·대출": ["LON"],
}
